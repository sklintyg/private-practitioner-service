...
public class MedgivandeBootstrapBean {
  ...
  private void addMedgivandeText(Resource res) {
    ...
    try {
      MedgivandeTextEntity medgivandeTextEntity = new CustomObjectMapper().readValue(
          res.getInputStream(),
          MedgivandeTextEntity.class);
      if (!medgivandeTextEntityRepository.existsById(medgivandeTextEntity.getVersion())) {
        medgivandeTextEntityRepository.save(medgivandeTextEntity);
      }
    } catch (IOException e) {
