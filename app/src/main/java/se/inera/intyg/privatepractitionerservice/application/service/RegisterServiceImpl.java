...
import se.inera.intyg.privatepractitionerservice.infrastructure.persistence.model.MedgivandeText;
...
import se.inera.intyg.privatepractitionerservice.infrastructure.persistence.repository.MedgivandeTextRepository;
import se.inera.intyg.privatepractitionerservice.infrastructure.persistence.repository.PrivatlakareIdRepository;
...
//    privatlakare.setFullstandigtNamn(userService.getUser().getName());
    ...
    monitoringService.logUserRegistered(privatlakare.getPersonId(), godkantMedgivandeVersion,
        privatlakare.getHsaId(), status);
    return status;
  }
  ...
  private String generateHsaId(PrivatlakareId privatlakareId) {
    ...
    return "SE165565594230-WEBCERT" + StringUtils.leftPad(Integer.toString(privatlakareId.getId()),
        5, '0');
    // CHECKSTYLE:ON MagicNumber
  }

  private Set<Medgivande> createMedgivandeSet(Long godkantMedgivandeVersion,
      Privatlakare privatlakare) {
    MedgivandeText medgivandeText = medgivandeTextRepository.findById(godkantMedgivandeVersion)
        .orElse(null);
