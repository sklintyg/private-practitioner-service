 ...
 */
package se.inera.intyg.privatepractitionerservice.infrastructure.persistence.config;

import static se.inera.intyg.privatepractitionerservice.infrastructure.persistence.config.PersistenceConfigBase.BASE_PACKAGES;
...
public class PersistenceConfigDev extends PersistenceConfigBase {
  ...
  Server h2WebServer() throws SQLException {
    ...
    final Server server = Server.createWebServer("-web", "-webAllowOthers", "-webPort",
        databaseHttpPort);
    server.start();
    return server;
  }
